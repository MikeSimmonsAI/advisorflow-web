...
const STATUS_COLORS = {
  active: 'var(--gm-teal)', trialing: 'var(--gm-blue)', past_due: 'var(--gm-amber)',
  unpaid: 'var(--gm-amber)', paused: 'var(--gm-amber)', incomplete: 'var(--gm-amber)',
  canceled: 'var(--gm-red)', incomplete_expired: 'var(--gm-red)',
};
...
const SETUP_TONE = {
  paid: ['var(--gm-teal)', 'Paid'],
  checkout_pending: ['var(--gm-amber)', 'Sent'],
  not_sent: ['var(--gm-dim)', 'Not sent'],
  failed: ['var(--gm-red)', 'Failed'],
};
...
function NoSource({ reason }) {
  return (
    <span title={reason || 'This figure is not being recorded anywhere yet.'}
          style={{ color: 'var(--gm-dim)', fontSize: 13, fontStyle: 'italic' }}>
      not tracked
    </span>
  );
}
...
function Tile({ label, value, sub, reason }) {
  return (
    <div style={{ ...CARD, minWidth: 0 }}>
      <div style={{ fontSize: 12, color: 'var(--gm-dim)', marginBottom: 6, textTransform: 'uppercase',
      ...
      {sub && <div style={{ fontSize: 12, color: 'var(--gm-dim)', marginTop: 6 }}>{sub}</div>}
    </div>
  );
}
...
export default function GodBillingOps() {
  ...
  return (
    <div style={{ padding: '28px', maxWidth: 1500, margin: '0 auto', color: 'var(--gm-blue)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
        ...
        <div>
          ...
          <p style={{ color: 'var(--gm-dim)', margin: '6px 0 0', fontSize: 14 }}>
      ...
      {err && (
        <div style={{ background: 'var(--gm-pill-red-bg)', border: '1px solid var(--gm-red)', borderRadius: 8,
                      padding: '12px 16px', margin: '16px 0', color: 'var(--gm-red)', fontSize: 14 }}>
          {err}
        </div>
      )}
      ...
      {mrr?.unpriced_organizations?.length > 0 && (
        <div style={{ ...CARD, borderColor: 'var(--gm-pill-amber-bd)', marginBottom: 24 }}>
          <div style={{ color: 'var(--gm-amber)', fontWeight: 700, fontSize: 14, marginBottom: 8 }}>
          ...
          {mrr.unpriced_organizations.slice(0, 6).map(u => (
            <div key={u.organization_id} style={{ fontSize: 13, color: 'var(--gm-amber)', marginTop: 4 }}>
      ...
      <div style={{ ...CARD, padding: 0, overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline',
                      padding: '16px 20px', borderBottom: '1px solid var(--gm-blue)', flexWrap: 'wrap', gap: 8 }}>
          <div style={{ fontWeight: 700, fontSize: 15 }}>
            {roster?.shown ?? 0} customer{(roster?.shown ?? 0) === 1 ? '' : 's'}
            {roster?.total_in_scope !== undefined && filter !== 'all'
              ? <span style={{ color: 'var(--gm-dim)', fontWeight: 400 }}> of {roster.total_in_scope} in scope</span>
              : null}
          </div>
          <div style={{ fontSize: 13, color: 'var(--gm-dim)' }}>
            {roster?.visible_mrr_cents !== null && roster?.visible_mrr_cents !== undefined
              ? <>Visible MRR <strong style={{ color: 'var(--gm-teal)' }}>
        ...
        {resyncNote && (
          <div style={{ padding: '10px 20px', fontSize: 13, color: 'var(--gm-blue)',
                        background: 'var(--gm-pill-blue-bg)',
                        borderBottom: '1px solid var(--gm-blue)' }}>
            {resyncNote}
          </div>
        )}
        ...
        {changeForm && (
          <div style={{ padding: '14px 20px', background: 'var(--gm-pill-blue-bg)',
                        borderBottom: '1px solid var(--gm-blue)' }}>
        ...
        {changePreview && (
          <div style={{ padding: '14px 20px', background: 'var(--gm-pill-amber-bg)',
                        borderBottom: '1px solid var(--gm-blue)' }}>
            ...
            <table style={{ fontSize: 12, borderCollapse: 'collapse',
              ...
              <tbody>
                {[
                  ...
                  <tr key={k}>
                    <td style={{ padding: '3px 16px 3px 0', color: 'var(--gm-dim)' }}>{k}</td>
            ...
            {changePreview.preview.commitment_changed && (
              <div style={{ fontSize: 12, color: 'var(--gm-amber)', fontWeight: 600,
                            marginBottom: 8 }}>
                This also changes their commitment.
              </div>
            )}
            <div style={{ fontSize: 12, color: 'var(--gm-dim)', marginBottom: 10,
              ...
              <tbody>
                {resyncPreview.changed.map(c => (
                  <tr key={c.field}>
                    ...
                    <td style={{ padding: '3px 10px 3px 0', color: 'var(--gm-dim)' }}>
                      {c.from ?? 'not set'}
                    </td>
                    <td style={{ padding: '3px 10px 3px 0', color: 'var(--gm-dim)' }}>→</td>
                    <td style={{ padding: '3px 0', color: 'var(--gm-teal)',
                                 fontWeight: 600 }}>
                      {c.to ?? 'not set'}
                    </td>
                  </tr>
        ...
        {loading ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--gm-dim)' }}>Loading…</div>
        ) : rows.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--gm-dim)', fontSize: 14 }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {rows.map(c => {
                  const sc = STATUS_COLORS[c.billing_status] || 'var(--gm-dim)';
                  return (
                    <tr key={c.organization_id} style={{ borderTop: '1px solid var(--gm-blue)' }}>
                      <td style={{ padding: '12px 16px', fontWeight: 600 }}>
                        {c.name}
                        {c.implementation_status && (
                          <div style={{ fontSize: 11, color: 'var(--gm-dim)', marginTop: 2 }}>
                      ...
                      <td style={{ padding: '12px' }}>
                        ...
                        {c.pending_plan && (
                          <div style={{ fontSize: 11, color: 'var(--gm-amber)', marginTop: 2 }}>
                        ...
                        {c.cancel_at_period_end && (
                          <div style={{ fontSize: 11, color: 'var(--gm-red)', marginTop: 2 }}>Cancelling</div>
                        )}
                      </td>
                      <td style={{ padding: '12px' }}>
                      ...
                      <td style={{ padding: '12px' }}>
                        {(() => {
                          if (!c.setup_fee) {
                            return <span style={{ color: 'var(--gm-ghost)' }}>—</span>;
                          }
                          const [tone, label] =
                            SETUP_TONE[c.setup_fee.status]
                            || ['var(--gm-dim)', String(c.setup_fee.status)];
                          return (
                            <>
                      ...
                      <td style={{ padding: '12px', textAlign: 'right',
                        ...
                        {c.addons_mrr_cents > 0 && c.mrr_cents != null ? (
                          <div style={{ fontSize: 11, color: 'var(--gm-teal)' }}
                            ...
                            <div style={{ color: 'var(--gm-dim)' }}>
                              = {money(c.total_mrr_cents, c.currency)}
                            </div>
                          </div>
                        ) : null}
                      </td>
                      <td style={{ padding: '12px', color: 'var(--gm-text)' }}>
                        {when(c.current_period_end) || '—'}
                        {c.trial_end && (
                          <div style={{ fontSize: 11, color: 'var(--gm-blue)' }}>
                            Trial to {when(c.trial_end)}
                          </div>
                        )}
                      </td>
                      <td style={{ padding: '12px', color: c.card_last4 ? 'var(--gm-text)' : 'var(--gm-dim)' }}>
                        {c.card_last4
                      ...
                      <td style={{ padding: '12px', textAlign: 'right',
                                   fontVariantNumeric: 'tabular-nums',
                                   color: c.held_lead_count > 0 ? 'var(--gm-amber)' : 'var(--gm-ghost)',
                      ...
                      <td style={{ padding: '12px', fontSize: 12 }}>
                        {c.commercial_source?.from_pipeline ? (
                          <>
                            <div style={{ color: 'var(--gm-teal)' }}>From pipeline</div>
                            {c.commercial_source.sales_rep && (
                              <div style={{ color: 'var(--gm-dim)' }}>{c.commercial_source.sales_rep}</div>
                            )}
                          </>
                        ) : (
                      ...
                      <td style={{ padding: '12px 16px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                        ...
                        {c.has_subscription && (
                          <button
                            ...
                            style={{ background: 'transparent', color: 'var(--gm-dim)',
                                     border: '1px solid var(--gm-blue)', borderRadius: 6,
                        ...
                        <Link to={`/god/customers/${c.organization_id}/360`}
                          style={{ color: 'var(--gm-blue)', textDecoration: 'none', fontWeight: 600 }}>
                          360 →
                        </Link>
                      </td>
                    </tr>
      ...
      <div style={{ ...CARD, marginTop: 24 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--gm-dim)', marginBottom: 14 }}>
        ...
        {!events?.events?.length ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {events.events.map(e => (
                  <tr key={e.id} style={{ borderTop: '1px solid var(--gm-blue)', color: 'var(--gm-text)' }}>
                    ...
                    <td style={{ padding: '10px 12px',
                                 color: e.outcome === 'duplicate' ? 'var(--gm-dim)' : 'var(--gm-text)' }}>
                      {e.outcome}
                      {e.detail && (
                        <div style={{ fontSize: 11, color: 'var(--gm-dim)' }}>{e.detail}</div>
                      )}
                    </td>
                    <td style={{ padding: '10px 12px', fontSize: 12 }}>
                      {e.earned_compensation
                        ? <span style={{ color: 'var(--gm-teal)' }}>earned</span>
                        : <span style={{ color: 'var(--gm-dim)' }}>
                            {e.compensation_note || '—'}
                          </span>}
                    </td>
