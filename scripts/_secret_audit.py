...
now lives. So every hit is reported as prefix + length + a verdict, and the
body of the token is never rendered.

VERDICTS
    LIVE-SHAPED   long enough and random enough to be a working credential
    PLACEHOLDER   contains an obvious filler marker (xxx, test, your, ...)
...
PATTERNS = [
    ...
    ("stripe",   re.compile(r"(?<![A-Za-z0-9_])(?:sk|rk)_live_[A-Za-z0-9]{20,}"), 28),
]
...
def verdict(label, token, minimum):
    low = token.lower()
...
def main(argv):
                ...
                for lineno, line in enumerate(fh, 1):
                    for label, pattern, minimum in PATTERNS:
                        for match in pattern.finditer(line):
                            token = match.group(0)
                            findings.append((
                                path, lineno, label, len(token),
                                token[:len(label) + 2].split("_")[0],
                                verdict(label, token, minimum)))
    ...
    live = [f for f in findings if f[5] == "LIVE-SHAPED"]
    for path, lineno, label, length, _prefix, v in sorted(findings):
        print("%-11s %-4s len=%-3d %s:%d" % (label, v, length, path, lineno))
    print()
