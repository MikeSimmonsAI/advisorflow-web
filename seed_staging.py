...
# Fetch actual org IDs (may have existed)
...
advisors = [
    # Restland
    (uid(), restland_id, 'james.hull@restland.test',    'James Hull',     pw(), 'advisor',   True, False, '+14695551001'),
    (uid(), restland_id, 'taffiney.m@restland.test',    'Taffiney Moore', pw(), 'advisor',   True, False, '+14695551002'),
    (uid(), restland_id, 'admin@restland.test',         'Dana Restland',  pw(), 'org_admin', True, False, '+14695551003'),
    # Harmony
    (uid(), harmony_id,  'ct@harmonylife.test',         'CT Wilson',      pw(), 'advisor',   True, False, '+14695552001'),
    (uid(), harmony_id,  'meava@harmonylife.test',      'Meava Jackson',  pw(), 'advisor',   True, False, '+14695552002'),
    (uid(), harmony_id,  'admin@harmonylife.test',      'Sam Harmony',    pw(), 'org_admin', True, False, '+14695552003'),
    # EvoSys
    (uid(), evosys_id,   'demo@evosyspro.test',         'Demo Advisor',   pw(), 'advisor',   True, False, '+14695553001'),
]
cur.executemany(
    """INSERT INTO users (id, organization_id, email, full_name, password_hash, role, is_active, must_change_password, phone)
       VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (email) DO NOTHING""",
    advisors
)

# Fetch advisor IDs by email
...
def random_phone():
    return f"+1469555{random.randint(1000,9999)}"

def random_email(fn, ln):
    return f"{fn.lower()}.{ln.lower()}{random.randint(10,99)}@testlead.example"

leads_data = []
# 40 leads for Restland, 35 for Harmony, 15 for EvoSys
...
execute_values(cur,
    """INSERT INTO leads
       (id, organization_id, first_name, last_name, phone, email,
        tier, engagement_temperature, status, assigned_to,
...
# ── 4. Messages (SMS activity) ────────────────────────────────────────────────
cur.execute("SELECT id, organization_id, assigned_to FROM leads LIMIT 60")
lead_rows = cur.fetchall()

messages = []
for lead_id, org_id, assigned_to in lead_rows:
    if not assigned_to: continue
    for j in range(random.randint(1, 4)):
        messages.append((
            uid(), lead_id, assigned_to,
            random.choice([
                "Hi {name}, this is James from Restland. Just wanted to follow up on your pre-arrangement inquiry. Do you have a few minutes this week?",
                ...
                "Hi there, I wanted to make sure you received the information we sent over. Happy to answer any questions.",
...
execute_values(cur,
    """INSERT INTO messages (id, lead_id, sender_id, body, twilio_status, sent_at)
       VALUES %s ON CONFLICT DO NOTHING""",
    messages
)
...
print("Orgs: Restland Funeral Home, Harmony Life Insurance, EvoSys Pro Demo Org")
print("Advisors: email format like james.hull@restland.test / password: Newpc!1me")
print("Leads: 90 total across 3 orgs")
