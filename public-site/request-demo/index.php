...
<div id="fallbackAction" class="fallback-action" hidden><button type="button" class="btn btn-ghost-gold" id="requestInstead">Send My Information Instead</button><div class="field-help">If online scheduling is unavailable, we can still receive your information and contact you directly.</div></div>
</div>
...
<div class="field full"><button class="btn btn-gold" type="submit" id="bookButton" disabled>Book My Discovery + Demo →</button><div class="form-meta">By booking, you agree that EvoSys Pro may contact you by email or phone regarding this meeting. SMS requires the separate optional consent checkbox above.</div></div></div>
</div>
</form></div>
...
function offline(message){availabilityReady=false;statusBox.textContent=message||'Online scheduling is temporarily unavailable.';statusBox.className='booking-status error';dateStrip.hidden=true;timeGrid.hidden=true;moreDates.hidden=true;nextOpen.hidden=true;startsAt.value='';selectedSlot.textContent='';bookButton.disabled=true;fallbackAction.hidden=false;}
...
form.addEventListener('submit',e=>{e.preventDefault();submit('book');});requestInstead.addEventListener('click',()=>submit('request'));moreDates.addEventListener('click',()=>{showAllDates=!showAllDates;renderDates();});loadAvailability();
})();
</script>
<?php site_footer(); ?>
