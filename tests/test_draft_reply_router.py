...
# ---------------------------------------------------------------------------
# Tone selection - per Mike's explicit request: "Suggest Reply" always
# produced one fixed soft tone with no way to ask for anything stronger.
# Now accepts soft/standard/urgent/direct, each genuinely changing what
# the AI is instructed to write, not just relabeling the same output.
# ---------------------------------------------------------------------------

def test_draft_reply_defaults_to_standard_tone_when_omitted(client, db_session, sample_lead, auth_headers, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    draft_reply_service._client = None
...
def test_draft_reply_rejects_invalid_tone(client, db_session, sample_lead, auth_headers):
    response = client.post(f"/sms/draft-reply/{sample_lead.id}", headers=auth_headers, json={"tone": "aggressive_yelling"})
    assert response.status_code == 400
    assert "tone must be one of" in response.json()["detail"]


def test_draft_reply_fallback_text_differs_by_tone(client, db_session, sample_lead, auth_headers, monkeypatch):
    """
    The actual point of this feature: different tones must produce
    genuinely different fallback text, not the same message with a
    tone label slapped on top.
    """
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    draft_reply_service._client = None
    ...
    soft_response = client.post(f"/sms/draft-reply/{sample_lead.id}", headers=auth_headers, json={"tone": "soft"})
    direct_response = client.post(f"/sms/draft-reply/{sample_lead.id}", headers=auth_headers, json={"tone": "direct"})

    assert soft_response.json()["suggested_reply"] != direct_response.json()["suggested_reply"]
    assert "no rush" in soft_response.json()["suggested_reply"].lower()
    assert "lock in" in direct_response.json()["suggested_reply"].lower()


def test_draft_reply_all_four_tones_produce_valid_fallback_text(client, db_session, sample_lead, sample_advisor, auth_headers, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    draft_reply_service._client = None
    ...
    for tone in ("soft", "standard", "urgent", "direct"):
        response = client.post(f"/sms/draft-reply/{sample_lead.id}", headers=auth_headers, json={"tone": tone})
        assert response.status_code == 200
        body = response.json()
...
def test_draft_reply_ai_prompt_includes_tone_specific_instruction(client, db_session, sample_lead, auth_headers, monkeypatch):
    """Confirms the actual prompt sent to the model changes per tone, not just the response copy."""
    captured_prompts = []
    ...
    response = client.post(f"/sms/draft-reply/{sample_lead.id}", headers=auth_headers, json={"tone": "direct"})

    assert response.status_code == 200
    assert len(captured_prompts) == 1
    assert "TONE: Direct" in captured_prompts[0]
    assert "confident closer" in captured_prompts[0]


def test_draft_reply_tone_returned_in_response_matches_request(client, db_session, sample_lead, auth_headers, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    draft_reply_service._client = None
    ...
    response = client.post(f"/sms/draft-reply/{sample_lead.id}", headers=auth_headers, json={"tone": "urgent"})

    assert response.json()["tone"] == "urgent"
